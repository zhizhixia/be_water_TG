...
ui/send_loop.py      → 多群组异步发送主循环（SendState 状态机）— Flask 与 Flet 共用
ui/message_manager.py → 每群组独立 MessageSelector 管理器 — Flask 与 Flet 共用
ui/app.py, config_form.py, control_panel.py, status_panel.py → **Flet 桌面 GUI（已废弃，未接入 main.py）**
```
...
`run.bat:26` 写死了 `D:\miniconda3\envs\be_water\python.exe`。修改或部署到其他机器时需要调整。
...
### Flet GUI 已废弃（影响测试）
`ui/app.py`、`config_form.py`、`control_panel.py`、`status_panel.py` 是基于 Flet 的桌面 GUI 残留代码，**未接入 `main.py`**（`main.py` 只走 Flask）。`flet` **不在 `requirements.txt`** 中，因此这些 ui 模块无法直接运行。`tests/test_control_panel.py` 依赖 Flet，需 `--ignore`（见上方命令）。`ui/send_loop.py` 和 `ui/message_manager.py` 是纯逻辑、与 GUI 解耦，仍被 Flask 路径复用——修改时不要引入 Flet 依赖。
